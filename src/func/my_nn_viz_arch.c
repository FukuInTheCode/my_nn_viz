...
static void draw_connection(my_shit_t *s, sfRenderWindow *window,\
                                double nlayer_vpad, sfVector2f pos)
{
    ...
    for (uint32_t k = 0; k < s->nn->dims[s->i + 1]; ++k) {
        ...
        sfVertex connection[] = {
            {pos, interpolate_color(start, end,\
                            s->nn->theta_arr[s->i].arr[k][s->j]), {0, 0}},
            {pos2, interpolate_color(start, end,\
                                s->nn->theta_arr[s->i].arr[k][s->j]), {0, 0}}
        };
...
static void draw_connections(sfRenderWindow *window, my_shit_t *s, sfVector2f pos)
{
